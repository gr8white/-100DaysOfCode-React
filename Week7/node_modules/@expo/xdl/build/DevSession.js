...
let startSession = exports.startSession = (() => {
  var _ref = _asyncToGenerator(function* (projectRoot, exp, platform, forceUpdate = false) {
    if (forceUpdate) {
      keepUpdating = true;
    }
    ...
    if (!(_Config || _load_Config()).default.offline && keepUpdating) {
      ...
      try {
        ...
        if (platform === 'native') {
          ...
          url = yield (_UrlUtils || _load_UrlUtils()).constructWebAppUrlAsync(projectRoot);
        } else {
      ...
      setTimeout(function () {
        return startSession(projectRoot, exp, platform);
      }, UPDATE_FREQUENCY_SECS * 1000);
    }
  });

  return function startSession(_x, _x2, _x3) {
